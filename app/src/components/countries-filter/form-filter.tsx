...
const FormFilter = ({ data }: Properties) => {
  ...
  const search = useCallback(() => {
    setFilteredData([...data.filter((country: any) => country.name.common === keyword || country.name.common.includes(keyword))])
  },[data, keyword])

  // Filter function
  const filter = useCallback((event: any) => {
    setFilteredData([...data.filter((country: any) => country.name.common === event.target.value)])
  },[data])
  ...
  return (
    <div className='mx-auto max-w-[62.5rem]'>
      ...
      {filteredData.length > 0 && (
        <>
          <h2 className='text-3xl lg:text-4xl font-extralight text-blue mt-10 mb-5'>Results:</h2>
              ...
              <tbody>
                {filteredData.map((country, index) => (
                  <tr key={`country-${index}`}>
                    ...
                    <td>{Object.keys(country.languages)[0] && Object.keys(country.languages)[0].toUpperCase()}</td>
                    <td>{Object.keys(country.currencies)[0] && Object.keys(country.currencies)[0]}</td>
